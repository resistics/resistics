...
they are rendered into the API reference by Sphinx.
...
Write NumPy-style docstrings. The opening summary should describe the useful
outcome rather than repeat the object's name. For a non-trivial callable,
document its ``Parameters``, ``Returns`` or ``Yields``, and ``Raises`` contract
where applicable. Keep types consistent with Python annotations. Constructors
are documented on their class; do not duplicate the class contract on
``__init__``. Conventional dunder methods need prose only when their behaviour
is surprising.
...
parameter and job construction, processing, result access, and plotting. Add
an example to a lower-level operation when its units, shapes, edge cases, or
composition are otherwise difficult to infer.

Use executable ``>>>`` examples wherever practical. The test suite runs
production docstrings as doctests, so examples must be deterministic and
small. Examples that require external data or interactive rendering should
still show the essential calls and clearly identify omitted setup.

Plots and related Sphinx directives stay with the function or class they
explain. Existing ``.. plot::``, ``.. note::``, ``.. warning::``, ``.. math::``,
cross-reference roles, and similar directives are protected documentation
assets, not text to move into a separate page. A future MyST conversion may
translate their syntax to an equivalent directive, but it must preserve their
location and rendered behaviour.

At the Checkpoint 2.4 boundary, production docstrings contain at least 86
...
the signature, annotations, and NumPy sections, including short docstrings.
The committed pydoclint baseline records inherited contract debt only. Do not
