"""
Calculate features for data in a project environment
Get features for data in a project environment
View features for data in a project environment
"""
import sys
import os
import numpy as np
...
from resitics.decimate.parameters import DecimationParameters
from resistics.statistics.features import Feature
...
#     project: ProjectData, site: str, meas: str, stat: str, declevel: int = 0, **kwargs
# ) -> StatisticData:
#     from resistics.statistics.io import FeatureIO

#     options = {}
#     options["specdir"] = project.config.configParams["Spectra"]["specdir"]
#     options = parseKeywords(options, kwargs)

#     siteData = project.getSiteData(site)
#     if not siteData:
...
def project_features(project: ProjectData, features: List[Feature], **kwargs):
    from resistics.statistics.io import StatisticIO

    options = {}
    options["sites"] = project.getSites()
    options["sampleFreqs"] = project.getSampleFreqs()
    options["chans"] = []
    options["specdir"] = project.config.configParams["Spectra"]["specdir"]
    # options["ncores"] = project.config.get_feature_cores()
    options = parseKeywords(options, kwargs)

    # projectText(
    #     "Calculating features: {} for sites: {}".format(
    #         listToString(options["stats"]), listToString(options["sites"])
    #     )
    # )
    # loop through sites and calculate statistics
    stat_io = StatisticIO()
    for site_name in options["sites"]:
        site = project.getSiteData(site_name)
        measurements = site.getMeasurements()
        for meas in measurements:
            measurement_features(project, site, meas, features, stat_io, options)
...
def measurement_features(
    ...
    fs_meas = site.getMeasurementSampleFreq(meas)
    if fs_meas not in options["sampleFreqs"]:
        return None
    projectText(
        "Calculating stats for site {}, measurement {}".format(site.siteName, meas)
    )
    dec_params = getDecimationParameters(fs_meas, project.config)
    nlevels = dec_params.numLevels
    spec_path = site.getMeasurementSpecPath(meas) / options["specdir"]
    spec_reader = SpectrumReader(spec_path)

    for level in range(nlevels):
        level_features(level, spec_reader, dec_params, options)


def level_features(
    level: int,
    spec_reader: SpectrumReader,
    dec_params: DecimationParameters,
    options: Dict[str, Any],
) -> None:
    check = spec_reader.openBinaryForReading("spectra", level)
    if not check:
        return
    refTime = spec_reader.getReferenceTime()
    winSize = spec_reader.getWindowSize()
    winOlap = spec_reader.getWindowOverlap()
    numWindows = spec_reader.getNumWindows()
    sampleFreqDec = spec_reader.getSampleFreq()
    evalFreq = dec_params.getEvalFrequenciesForLevel(level)

    # dictionary for saving statistic data
    statData = {}
    for stat in options["stats"]:
        statElements = getStatElements(stat)
        statData[stat] = StatisticData(stat, refTime, sampleFreqDec, winSize, winOlap)
        statData[stat].setStatParams(numWindows, statElements, evalFreq)
        statData[stat].comments = specReader.getComments()
        statData[stat].addComment(project.config.getConfigComment())
    ...
    if options["ncores"] > 0:
        ...
        for iW in range(numWindows):
            ...
            for stat in options["stats"]:
                statData[stat].addStat(iW, globalIndices[iW], winStatData[stat])
...
# def calculateRemoteStatistics(project: ProjectData, remoteSite: str, **kwargs):
...
#     options["chans"] = []
#     options["specdir"] = project.config.configParams["Spectra"]["specdir"]
#     options["remotestats"] = project.config.configParams["Statistics"]["remotestats"]
#     options["ncores"] = project.config.getStatisticCores()
#     options = parseKeywords(options, kwargs)
...
#             decParams = getDecimationParameters(sampleFreq, project.config)
#             numLevels = decParams.numLevels
#             winParams = getWindowParameters(decParams, project.config)
#             # create the window selector and find the shared windows
#             winSelector = getWindowSelector(project, decParams, winParams)
...
def viewStatistic(
    project: ProjectData, site: str, sampleFreq: Union[int, float], stat: str, **kwargs
) -> Union[Figure, None]:
    ...
    maskWindows = []
    if options["maskname"] != "":
        maskData = getMaskData(project, site, options["maskname"], sampleFreq)
...
def viewStatisticHistogram(
    project: ProjectData, site: str, sampleFreq: float, stat: str, **kwargs
) -> Union[Figure, None]:
