...
def get_eval_freqs_min(fs: float, f_min: float) -> np.ndarray:
    ...
    :param f_min: Minimum allowable frequency

    :return: Array of evaluation frequencies
...
def get_eval_freqs_size(fs: float, n_freqs: int) -> np.ndarray:
    ...
    :param fs: Sampling frequency
    :param n_freqs: Number of evaluation frequencies
...
def get_eval_freqs(
    ...
    :param f_min: Minimum cutoff for evaluation frequencies, by default None
    :param n_freqs: Number of evaluation frequencies, by default None

    :return: Evaluation frequencies array
...
class DecimationParameters(ResisticsModel):
    ...
    :param per_level: Evaluation frequencies per level
    :param min_samples: Number of samples to under which to quit decimating
    :param eval_df: The DataFrame with the decimation information
    ...
    **Examples**
    ...
    >>> print(dec_params.to_dataframe()) # doctest: +NORMALIZE_WHITESPACE
    ...
    def set_derived_decimation_fields(self) -> "DecimationParameters":
        ...
        :return: Validated parameters with increments and level rates populated.
        """
    ...
    def __getitem__(self, args: int | tuple[int, int]) -> list[float] | float:
        """Get evaluation frequencies by level and optional frequency index.

        :param args: Level index, or a ``(level, frequency index)`` pair.

        :return: All frequencies for a level or one selected frequency.
        """
    ...
    def check_eval_idx(self, idx: int) -> None:
        """Check an evaluation-frequency index.

        :param idx: Frequency index within a decimation level.

        :raises ValueError: If the index is outside the configured range.
        """
    ...
    def get_eval_freqs(self, level: int) -> list[float]:
        ...
        :return: List of evaluation frequencies
        """
    ...
    def get_eval_freq(self, level: int, idx: int) -> float:
        ...
        :param idx: Evaluation frequency index

        :return: The evaluation frequency
        """
        self.check_level(level)
    ...
    def get_incremental_factor(self, level: int) -> int:
        ...
        :return: The incremental decimation factor
        """
        self.check_level(level)
        return self.dec_increments[level]

    def to_numpy(self) -> np.ndarray:
        """Get evaluation frequencies as a two-dimensional array.

        :return: Frequencies shaped as levels by frequencies per level.
        """
        return np.array(self.eval_freqs).reshape(self.n_levels, self.per_level)

    def to_dataframe(self) -> pd.DataFrame:
        """Provide decimation parameters as a dataframe.

        :return: Frequencies, sample rates, and factors indexed by level.
        """
...
class DecimationSetup(ResisticsProcess):
    ...
    :param n_levels: Number of decimation levels, by default 8
    :param per_level: Number of frequencies per level, by default 5
    :param min_samples: Number of samples under which to quit decimating
    :param div_factor: Minimum division factor for decimation, by default 2.
    :param eval_freqs: Explicit definition of evaluation frequencies as a flat list, by
    ...
    **Examples**
    ...
    def execute(self, inputs: dict[str, Any], context: Any) -> DecimationParameters:
        ...
        :param inputs: Flow inputs containing ``time_data``.
        :param context: Runtime context; unused by this process.

        :return: Decimation parameters for the input sampling frequency.
        """
        del context
    ...
    def run(self, fs: float) -> DecimationParameters:
        ...
        :return: Decimation parameterisation
        """
    ...
    def _get_eval_freqs(self, fs: float) -> np.ndarray:
        ...
        :return: Evaluation frequencies as a flat array

        :raises ValueError: If the frequency count is invalid or any frequency exceeds Nyquist.
        """
        nyquist = fs / 2
    ...
    def _get_factors(self, fs: float, eval_freqs: np.ndarray) -> list[int]:
        ...
        :param fs: Sampling frequency, Hz
        :param eval_freqs: Evaluation frequency as flat array

        :return: **factors** — The decimation factors referenced to the sampling frequency
        """
    ...
    def _get_decimation_factor(self, fs: float, f_max: float) -> int:
        ...
        ```
        :param fs: Sampling frequency, Hz
        :param f_max: Maximum evaluation frequency in the decimation level
...
class DecimatedData(ResisticsData):
    ...
    :param data: The decimated time data

    **Examples**

    ```{plot}
    :width: 90%
    :filename-prefix: decimated-data
    ...
    def __init__(
        self, metadata: DecimatedMetadata, data: dict[int, np.ndarray]
    ) -> None:
    ...
    def get_level(self, level: int) -> np.ndarray:
        ...
        :return: The data for the decimation level
    ...
    def to_string(self) -> str:
        ...
        :return: Human-readable per-level sampling and time metadata.
        """
...
class Decimator(ResisticsProcess):
    ...
    def run(
        ...
        :param time_data: TimeData to decimate
    ...
    def _decimate(self, time_data: TimeData, factor: int) -> TimeData:
        """Decimate time data using the IIR decimator.

        :param time_data: Time data at the preceding level.
        :param factor: Incremental decimation factor.

        :return: Decimated time data, or the original object for factor one.
        """
    ...
    def _resample(self, time_data: TimeData, factor: int) -> TimeData:
        """Decimate time data using polyphase resampling.

        :param time_data: Time data at the preceding level.
        :param factor: Incremental resampling factor.

        :return: Resampled time data, or the original object for factor one.
        """
    ...
    def _get_metadata(
        ...
        """Get metadata for the decimated data.

        :param metadata_dict: Source time-data metadata.
        :param levels_metadata: Metadata for each completed decimation level.

        :return: Aggregate decimated-data metadata.
        """
...
class DecimatedDataWriter(ResisticsWriter):
    ...
    def run(self, dir_path: Path, data: ResisticsData) -> None:
        ...
        :param data: Decimated data to write out

        :raises TypeError: If ``data`` is not decimated data.
...
class DecimatedDataReader(ResisticsProcess):
    ...
    def run(
        ...
        :return: DecimatedData or DecimatedMetadata if metadata_only
